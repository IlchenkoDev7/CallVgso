...
class BaseModel implements \Iterator
{
    ...
    function __construct()
    {
        if (!self::$connection)
        {
            self::$connection = \Modules\Helpers\connectToDb();
    ...
    public function getRecordWithLimit($fields = '*', $links = NULL, $where = '', $params = NULL, $order = '', $offset = NULL, $limit = NULL)
    {
        $this->record = FALSE;
        $this->select($fields, $links, $where, $params, $order, $offset, $limit);
        return $this->query->fetch(\PDO::FETCH_ASSOC);      
    }
